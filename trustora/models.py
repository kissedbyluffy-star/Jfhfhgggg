...
class Escrow(Base):
    ...
    amount_expected: Mapped[float] = mapped_column(Numeric(18, 6))
    amount_received: Mapped[float | None] = mapped_column(Numeric(18, 6))
    fee_snapshot_json: Mapped[dict] = mapped_column(JSON)
    fee_amount: Mapped[float] = mapped_column(Numeric(18, 6))
    net_amount: Mapped[float] = mapped_column(Numeric(18, 6))
    ...
    fee_tx_hash: Mapped[str | None] = mapped_column(String(128))
    ...
    __table_args__ = (
        ...
        UniqueConstraint("chain", "deposit_address", name="uq_chain_deposit_address"),
    )
