...
async def handle_payout(request: web.Request) -> web.Response:
    ...
    await ensure_gas(app, chain_enum, deposit_address)
...
async def ensure_gas(app: web.Application, chain: Chain, address: str) -> None:
    key = f"gas_funded:{chain.value}:{address}"
    if await app["redis"].get(key):
        return
    if chain == Chain.TRC20:
        client = Tron(network="mainnet", provider=HTTPProvider(app["tron_rpc"][0]))
        balance = client.get_account_balance(address)
        if balance >= app["settings"].tron_gas_min_balance:
            return
        await fund_tron_gas(app, address)
    else:
        web3 = Web3(Web3.HTTPProvider(app["bsc_rpc"][0]))
        balance = web3.eth.get_balance(address)
        threshold = Web3.to_wei(app["settings"].bsc_gas_min_balance, "ether")
        if balance >= threshold:
            return
        await fund_bsc_gas(app, address)
    await app["redis"].set(key, "1", ex=60 * 60 * 24 * 7)
