...
- **signer**: Only component with encrypted keys; signs & broadcasts payouts and funds gas at payout time.
...
Gas funding happens only at payout time. Configure `TRON_GAS_MIN_BALANCE`/`BSC_GAS_MIN_BALANCE` and funding amounts for your hot gas wallets.
...
- Underpaid escrows require admin resolution; do not top up the same deposit address.
