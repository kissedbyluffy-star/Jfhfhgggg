...
async def show_summary(callback: CallbackQuery, session_factory) -> None:
    ...
    if escrow.status == EscrowStatus.UNDERPAID:
        text += "\n⚠️ Underpaid escrows require admin resolution. Do not send top-ups to same address."
...
async def approve_and_send_payout(
    ...
    if callback:
        ...
        await callback.message.answer(
            "Completed (pending confirmations).\n"
